...
impl Processor {
    ...
    pub fn cycle(&mut self) {
        ...
        match opcode & 0xF000 {
            0x0000 => match opcode & 0x00FF {
                ...
                0x00EE => self.op_00ee(),
                0x0000 => {}
                _ => panic!("Unsupported opcode: {:#06X}", opcode,),
            },
            ...
            0x8000 => match self.opcode & 0x000F {
                ...
                0x0007 => self.op_8xy7(),
                0x000E => self.op_8xye(),
                _ => panic!("Unrecognized opcode: {:#06X}", opcode),
            },
            ...
            0xF000 => match opcode & 0x00FF {
                ...
                0x0055 => self.op_fx55(),
                0x0065 => self.op_fx65(),
                _ => println!("Unsupported opcode: 0x{:#06X}", opcode),
            },

            _ => panic!(
                "Unsupported opcode: 0x{:04X}::0x{:04X}",
                opcode,
                opcode & 0xF000
            ),
        }
    ...
    pub fn op_fx29(&mut self) {
        ...
        self.index = Self::FONTSET_START_ADDR as u16 + (5 * value as u16);
    }
    ...
    pub fn op_dxyn(&mut self) {
        ...
        for row in 0..height {
            ...
            for col in 0..8 {
                ...
                let display_index = u16::from(u16::from(y_pos) + row) * Self::DISPLAY_WIDTH
                    + u16::from(x_pos)
                    + col;
    ...
    pub fn op_cxkk(&mut self) {
        ...
        self.registers[register_idx as usize] = rng.gen::<u8>();
    }
    ...
    pub fn op_8xy4(&mut self) {
        ...
        self.registers[a_idx as usize] = sum as u8 & 0xFF;
    }
